...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <p style="color: #666; line-height: 1.5;">
        Your subscription is expiring in 3 days. Please renew it to continue using our services.
    </p>
...
export const processCronInstaResponseJob = async () => {
  ...
  for (const user of users) {
    if (user.planActive) {
      await insta_script(user.IG_USERNAME, user.IG_PASSWORD, user.messageLogicList);
    } else {
      continue;
    }
  }
...
export const processCronCheckSubscriptionsJob = async () => {
  ...
  for (const user of users) {
    if (user?.planActive && user?.expiresAt < now) {
      ...
      await sendEmail(user.email, emailSubjectExpriringSoon, emailBodyExpriringSoon);
    }
  }
};
