...
export async function createK8sDeployment(uniqueId, APP_BASE_URL = "https://mom-ai-restaurant.lat") {
    ...
    // Ensure KUBECONFIG_BASE64 is set
    const kubeConfigBase64 = process.env.KUBECONFIG_BASE64?.trim();
    if (!kubeConfigBase64) {
        throw new Error("Environment variable KUBECONFIG_BASE64 is not set");
    }

    // Validate Base64
    const base64Regex = /^[A-Za-z0-9+/=]+$/;
    if (!base64Regex.test(kubeConfigBase64)) {
        throw new Error("Invalid Base64 string in KUBECONFIG_BASE64");
    }

    // Decode Base64
    let kubeConfigContent;
    try {
        kubeConfigContent = Buffer.from(kubeConfigBase64, 'base64').toString('utf-8');
        console.log(`kubeConfigContent: ${kubeConfigContent}`);
    } catch (error) {
    ...
    try {
        ...
        console.log(`Deploymen ${deploymentName} created.`);
    } catch (error) {
