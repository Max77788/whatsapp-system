...
export async function POST(req) {
  try {
    ...
    console.log(
      `fromNumber: ${fromNumber}, toNumbers: ${toNumbers}, message: ${message}`,
    );
    ...
    const user = session
      ? await find_user({ email: session.user.email })
      : await find_user({ apiKey });
    ...
    for (const toNumber of toNumbersValue) {
      const lead = leads?.find(
        (lead) =>
          lead.phone_number === toNumber ||
          lead.phone_number.includes(toNumber.replace(/^0/, "")),
      );
